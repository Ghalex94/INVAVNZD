        ...
        public void mostrarTabla(DataGridView dgv)
        {
            ...
            try
            {
                using (var connection = GetConnection())
                {
                    ...
                    {
                        command.Connection = connection;
                        command.CommandText = "SELECT distinct id_usu, nombre_usu, usu, contra, tipo, case when tipo = 0 then 'Super Administrador' when tipo = 1 then 'Administrador' when tipo = 2 then 'Vendedor' end as tipo2, permisos, estado from tb_usuario where estado = 1";
                        command.CommandType = System.Data.CommandType.Text;
